#!/bin/bash
#SBATCH --job-name="ssast-ls-probe-asr"
#SBATCH --nodes=1
#SBATCH --gres=gpu:1
...
echo "Generating JSON files and vocab..."
...
exp_dir=./exp/probe-asr-${dataset}-f${fstride}-t${tstride}-b${batch_size}-lr${lr}-${task}-${model_size}-${pretrain_model}

echo "Starting Probe ASR Training..."

CUDA_CACHE_DISABLE=1 python -W ignore ../../run.py --dataset ${dataset} --data-train ${tr_data} --data-val ${val_data} --data-eval ${eval_data} --exp-dir $exp_dir --label-csv ${label_csv} --n_class ${n_class} --lr $lr --n-epochs ${epoch} --batch-size $batch_size --save_model True --freqm $freqm --timem $timem --mixup ${mixup} --bal ${bal} --tstride $tstride --fstride $fstride --fshape ${fshape} --tshape ${tshape} --warmup True --task ${task} --model_size ${model_size} --adaptschedule True --pretrained_mdl_path ${pretrain_path} --dataset_mean ${dataset_mean} --dataset_std ${dataset_std} --target_length ${target_length} --num_mel_bins 128 --head_lr 1 --noise ${noise} --lrscheduler_start 5 --lrscheduler_step 1 --lrscheduler_decay 0.85 --wa False --loss CTC --metrics wer
