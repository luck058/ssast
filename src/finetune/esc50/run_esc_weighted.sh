#!/bin/bash
#SBATCH --job-name="ssast-esc50-weighted"
#SBATCH --nodes=1
#SBATCH --gres=gpu:1
...
task=ft_avgtok_weighted
model_size="${2:-tiny}"
head_lr=1

base_exp_dir=./exp/test01-${dataset}-f$fstride-t$tstride-b$batch_size-lr${lr}-${task}-${model_size}-${pretrain_model}-${head_lr}-noise${noise}
